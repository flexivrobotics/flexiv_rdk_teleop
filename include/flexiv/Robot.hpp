 ...
 * IO methods, plus add-on methods for teleoperation. This interface is also
...
public:
    ...
    /**
     * @brief Send command and anonymous algorithm data to the remote robot for
     * teleoperation, so that the remote robot follows the motion of the local
     * robot/device. Latency is automatically handled so the commands don't have
     * to be real-time. However, real-time commands at 1kHz can maximize the
     * performance while latency leads to performance degradation.
     * @param[in] M1 Anonymous algorithm data M1 included in LOCAL robot's
     * RobotStates, to be sent to REMOTE robot.
     * @param[in] P1 Anonymous algorithm data P1 included in LOCAL robot's
     * RobotStates, to be sent to REMOTE robot.
     * @param[in] P2 Anonymous algorithm data P2 included in LOCAL robot's
     * RobotStates, to be sent to REMOTE robot.
     * @warning For safety and stability consideration, the remote robot is
     * velocity-controlled, thus, deviation on the absolute pose of local and
     * remote robot is expected to accumulate over time. The deviation can be
     * eliminated with pose synchronizations by commanding both robots to the
     * same Cartesian pose or joint positions using primitives like MoveL,
     * MoveJ, Home, etc.
     * @note Applicable modes: MODE_TELEOP_REMOTE.
     ...
     */
    void commandToRemote(const std::vector<double>& M1, const double P1,
        const std::vector<double>& P2);

    /**
     * @brief Send command and anonymous algorithm data to the local robot for
     * teleoperation, so that the external force/moment from the remote robot is
     * rendered on the local robot. Latency is automatically handled so the
     * commands don't have to be real-time. However, real-time commands at 1kHz
     * can maximize the performance while latency leads to performance
     * degradation.
     * @param[in] targetTcpWrench \f$ \mathbb{R}^{6 \times 1} \f$ target TCP
     * wrench to be rendered on the local robot, expressed in base frame.
     * \f$ \mathbb{R}^{3 \times 1} \f$ force and \f$ \mathbb{R}^{3 \times 1} \f$
     * moment: \f$ [f_x, f_y, f_z, m_x, m_y, m_z]^T~[N][Nm] \f$.
     * @param[in] P1 Anonymous algorithm data P1 included in REMOTE robot's
     * RobotStates, to be sent to LOCAL robot.
     * @param[in] P2 Anonymous algorithm data P2 included in REMOTE robot's
     * RobotStates, to be sent to LOCAL robot.
     * @param[in] P3 Anonymous algorithm data P3 included in REMOTE robot's
     * RobotStates, to be sent to LOCAL robot.
     * @note Applicable modes: MODE_TELEOP_LOCAL.
     * @throw InputException if input is invalid.
     * @throw LogicException if robot is not in the correct operation mode.
     * @throw ExecutionException if error occurred during execution.
     */
    void commandToLocal(const std::vector<double>& targetTcpWrench,
        const double P1, const std::vector<double>& P2,
    ...
    //============================ ROBOT OPERATION =============================
    /**
     * @brief Get general robot states like joint position, velocity, torque,
     * TCP pose, velocity, etc., plus add-on states for teleoperation.
